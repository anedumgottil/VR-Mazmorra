...
*/
    ...
    public void loadTiles(List<Tile> tilePool, string path) {
        ...
        foreach (GameObject tile in tilez) {
            ...
            if (i == 0 && tile.name.Contains ("_00") && generateTilePrefabsFromFile) {
                //this is the master prefab, the 00 prefab, the one we'll used to generate all the other prefabs, one prefab to rule them all
                TilePrefabGenerator tfg = new TilePrefabGenerator (tile, tile.name.Substring (0, tile.name.Length - 3));//takes name like "SciFiTiles_00" and makes it "SciFiTiles", see why naming convention is so important for Tiles?
                tfg.generatePrefabs (0, 20, mainTilePath, overwritePrefabsDuringGeneration);
                //tfg.generatePrefabs (0, 20, altTilePath); DEBUG: please uncomment
            }
